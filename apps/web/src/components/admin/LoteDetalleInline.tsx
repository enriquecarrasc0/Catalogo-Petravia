...
function ApartarVendedor({ loteId }: { loteId: string }) {
  ...
  // El admin puede apartar a nombre de CUALQUIER cliente (de cualquier
  // vendedor), no solo los propios — por eso usa /admin/clientes en vez
  // de /auth/tokens (que solo lista los clientes del vendedor en sesión).
  const { data: clientes = [] } = useQuery({
    queryKey: ['admin-clientes-mini'],
    queryFn: async (): Promise<ClienteOpcion[]> => {
      const res = await fetch(`${BASE}/admin/clientes`, { headers: vendedorHeaders() });
